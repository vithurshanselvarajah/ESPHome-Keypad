...
static bool restore_slot(esphome::uart::UARTComponent *uart, int slot, const std::string &b64) {
  ...
  while (off < tmpl.size()) {
    ...
    std::vector<uint8_t> pkt = {
        0xEF, 0x01, 0xFF, 0xFF, 0xFF, 0xFF, pkg_id, (uint8_t) (len >> 8), (uint8_t) (len & 0xFF)};
  ...
  if (!read_pkt(uart, pid, conf, data, 8000) || conf != 0x00) {
    ESP_LOGE("fp_backup", "Store slot %d %s: conf=0x%02X", slot,
             conf == 0xFF ? "TIMEOUT" : "failed", conf);
    return false;
  }
